...
public class HashSetRepository {
    ...
    synchronized private  boolean exist (Room room){
        try (Connection connection = DriverManager.getConnection(URL, username, password);){
            ...
            pst.setString(1, room.user);
            pst.setInt(2, room.area);
            pst.setString(3, room.name);
            pst.setInt(4, room.amount_of_furniture);
            ...
            int cost2 = 0;
            for (int i = 0; i < room.amount_of_furniture; i++) cost2 += room.furniture[i].cost;
    ...
    synchronized String  add (Room room){
        try (Connection connection = DriverManager.getConnection(URL, username, password);){
            ...
            if(this.exist(room) == true) return "Данный элемент был добавлен ранее";
            //обновляем таблицу rooms
            PreparedStatement statement1 = connection.prepareStatement(insert_into_rooms);
            statement1.setString(1, room.user);
            statement1.setString(2, room.name);
            statement1.setInt(3,room.amount_of_furniture);
            statement1.setInt(4,room.area);
            //statement1.setString(5, room.time.toString());
    ...
    synchronized String remove (Room room){
        ...
        try (Connection connection = DriverManager.getConnection(URL, username, password);){
            ...
            if(rs.next()){
                int id = rs.getInt("id");
                sql = "delete from rooms where id="+Integer.toString(id);
                Statement statement1 = connection.createStatement();
                statement1.executeUpdate(sql);
                sql = "delete from furniture where room_id="+Integer.toString(id);
                Statement statement2 = connection.createStatement();
    ...
    synchronized String add_if_min (Room room){
        try (Connection connection = DriverManager.getConnection(URL, username, password);){
            ...
            if (min_area > room.area) return this.add(room);
            else if (min_area == room.area){
                sql = "select max(amount_of_furniture) from rooms where area = " + Integer.toString(room.area);
                ...
                if (room.amount_of_furniture > max_amount) return this.add(room);
            }
        }
        catch (SQLException e){
            return "Неудача";
    ...
    synchronized String remove_greater (Room room){
        try (Connection connection = DriverManager.getConnection(URL, username, password);){
            ...
            pst.setString(1, room.user);
            pst.setInt(2, room.area);
            pst.setInt(3, room.amount_of_furniture);
            pst.executeUpdate();
